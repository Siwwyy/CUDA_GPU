...
// Test if peer to peer access is possible between GPUs.
int testPeerToPeer(int GPUCount);

// Multi GPU Gaussian elimination wrapper function.
void gaussianEliminationMultiGPU(unsigned int* packedTransposeAB, int rowCount, int columnCount);
void bcastPivotRow(int pivotPart, int myPart, int numGPUs, stSolverState *solverState);
